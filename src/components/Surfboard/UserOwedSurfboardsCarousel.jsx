...
import { auth } from "../../utils/firebase"; // Import Firebase auth to get the current user
import useSurfboardStore from "../../store/useSurfboardStore";
import SurfboardCard from "./SurfboardCard";
import LoadingIndicator from "../UI/LoadingIndicator";

function UserOwedSurfboardsCarousel() {
  const { surfboards, loading, fetchUserSurfboards, deleteSurfboard } =
    useSurfboardStore();

  useEffect(() => {
    const user = auth.currentUser;
    if (user) {
      fetchUserSurfboards(user.uid); // Fetch surfboards for the logged-in user
    }
  }, [fetchUserSurfboards]);
  ...
  return (
    <div>
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4 sm:align-center justify-items-center">
...
export default UserOwedSurfboardsCarousel;
